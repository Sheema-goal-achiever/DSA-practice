...
class Solution {
    public boolean isSafe(int idx,int[] flowerbed)
    {
       if(idx>0 && flowerbed[idx-1]==1 ) return false;
       if(idx<flowerbed.length-1 && flowerbed[idx+1]==1) return false;
       return true;
    }
    public boolean canPlaceFlowers(int[] flowerbed, int n) {
        ...
        {
            if(flowerbed[i]==0 && isSafe(i,flowerbed))
            {
                flowerbed[i]=1;
                count++;
